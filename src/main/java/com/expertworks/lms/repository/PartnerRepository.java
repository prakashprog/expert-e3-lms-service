...
public class PartnerRepository {
	...
	public List<Partner> queryOnsk(String groupId) {
		...
		DynamoDBQueryExpression<Partner> queryExpression = new DynamoDBQueryExpression<Partner>()
				.withIndexName("sk-index").withKeyConditionExpression("sk= :val1").withExpressionAttributeValues(eav)
	...
	public List<Partner> get(String partnerId) {
		...
		DynamoDBQueryExpression<Partner> dynamoDBQueryExpression = new DynamoDBQueryExpression()
				.withHashKeyValues(partner);
	...
	public Partner load(String partnerId, String rangeKey) {
		return dynamoDBMapper.load(Partner.class, partnerId, rangeKey);
	}
	 ...
	 * dynamoDBMapper.save(partnerId, new
	 * DynamoDBSaveExpression().withExpectedEntry("teamId", new
	 * ExpectedAttributeValue(new AttributeValue().withS(partnerId)))); return
	 * partnerId; }
	 */
	...
	public void update(Partner partner) {
		...
		dynamoDBMapper.save(partner, dynamoDBMapperConfig);
	}
